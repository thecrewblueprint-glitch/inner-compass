...
The harness never calls Gemini or OpenRouter. It sends every request to the local
`/api/eval/guidance` endpoint with `mode=deterministic` and `skipGemini=true`.
